    ...
    <div className="w-full max-w-[600px] mx-auto p-4 space-y-4 font-mono select-none">
      ...
      <div className="space-y-4">
        {loggedExercises.length === 0 ? (
          <div className="p-8 border-2 border-dashed border-pixel-border/80 bg-surface/80 text-center space-y-3">
            <div className="w-12 h-12 border border-[#00ff41]/50 bg-black text-[#00ff41] flex items-center justify-center mx-auto shadow-neon">
              <Sparkles className="w-6 h-6 animate-pulse" />
            </div>
            <div className="space-y-1">
              <h4 className="font-headline font-black text-base text-white uppercase tracking-wider">
                NO EXERCISES LOGGED YET
              </h4>
              <p className="text-xs text-zinc-400 max-w-sm mx-auto">
                BUILD YOUR WORKOUT RAID! CLICK THE BUTTON BELOW TO SELECT EXERCISES FROM THE DATABASE.
              </p>
            </div>
          </div>
        ) : (
              ...
              <div className="flex-1 overflow-y-auto space-y-3 pr-1 pt-1 min-h-[220px]">
                {filteredDatabase.length === 0 ? (
                  ...
                  filteredDatabase.map((def) => (
                    <div
                      key={def.id}
                      className="p-3 border border-pixel-border/60 bg-black/80 hover:border-[#00ff41] transition-all space-y-3"
                    >
